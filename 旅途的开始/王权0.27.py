...
# ==================== 场景函数 ====================
def scene_toss():
    DELAY.sleep(DELAY.SLOW)
    pm("你向上抛掷了一枚硬币，这枚硬币飞至空中", 0.75)
    pm("然后反向落入水中", 0.75)
    pm("产生涟漪", 0.7)
    pm("共振", 0.7)
    pm("扩散", 0.7)
    ...
    descriptions = [
        ...
        ("正常人的梦境处于照光层，和现实有所映照；一旦进入弱光层，就多少会获得一些能力，最常见的是能看见某些东西，以及获得少部分灵力。", 4),
        ("无光层的深度对常人而言，是足以致命的，这个级别的深度，往往寄宿着很多非人之物常人魂魄一旦进入无光层，就有被鬼神入侵的危险；深渊层，更加致命的深度，该领域长期徘徊某些精神构造体，譬如说'魔鬼'，譬如说'监视者'，瀛洲数量众多的鬼神，神州的神话古物，皆属于该层级，他们不会轻易死去，只是被历史遗忘了。", 5),
        ("而往后的，基本上不会有人潜入。", DELAY.VERY_SLOW),
        ("灵界无比危险，深渊之下，藏着的是无尽虚无，还是灵长类的综合意识阿赖耶，也根本无人可知。", DELAY.CHAPTER),
        ("不过，这也并不重要。", DELAY.VERY_SLOW),
        ("如今你也没能力潜入无光层以下，你也是预计到了这个情况，所以留下的坐标锚点，位于弱光层，好处是容易找得到，坏处嘛……", 4),
        ("待会儿就知道了。", DELAY.VERY_SLOW),
        ("前世你在灵界中打下了三个锚点，两个在弱光层，一个在无光层。", 2.5),
    ]
...
def scene_anchor():
    ...
    pm("第一道是留给自己的；第二道是黑色蔷薇会的后门；第三道……", 2.5)
    pm("第一道锚点，它藏匿的是一道残留的权能碎片。", 2.5)
...
def scene_bend_knees():
    ...
    pm("可五百年岁月流转而过，没人能抵得过岁月的磨损。", 2.5)
    pm("留在这里的不过是一件死物，一件保险。", DELAY.LONG)
    pm("为了防止自己没死成而做了一道保险，给自己留下了一点帝王时代的老本，方便之后直接冲去教国找教皇玩摔角，一命换一伤还是做得到的，尽量给同盟国拉扯一点发育时间。", 5)
    pm("只是没想到莉莉安奴这么争气，面对不死的帝王，反手一剑创世螺旋碎片当场给他扬了骨灰，于是这件临时的保险再也没派上用场，就这么一直留到了五百年后。", 5)
...
def scene_black_rose():
    ...
    pm("警告：启用该权能碎片将会导致灵界海洋暴动，产生多次回响，后果难以预知", 2.5)
    pm("如果执意发动权能，请通过最终验证", 2.5)
...
def epilogue():
    ...
    scenes = [
        ...
        ("幕府便是幕府，将军就是将军，世代承袭的名号，早已不需要前缀，更不需要似乎血统高贵的天皇世家的认同，血统并非最重要的条件，继子也可以继承。", 4.5),
        ...
        ("今日的将军也一如既往，祂有时什么事都不用做，只坐在天守阁最高层，迎接幕府上下的朝拜即可。", 4),
        ("结束后，祂便换了个方向，面朝大海，拿起彩色的团子，慢条斯理的咬上一口，眼中静默着排遣着无聊。", 4),
        ...
        ("'夏莎部长，您不是正在公国度假？这个点还不休息？'", 2.5),
        ("'怎么还睡得着！'血裔的执政官脸色没有半点血色，苍白着脸色，难看至极，甚至没时间遮掩自己的胸有沟壑：'我刚刚做了一场梦，我的梦境一向很准。'", 4.5),
        ("炎魔若有所思，点头说：'于是，你梦到什么了？'", 2.5),
        ("'我长话短说，我在梦里，看见了一双黄金瞳！'", 2.5),
        ("'诸位，或许我们现在要面对的，不仅是教国，还有至今都并未完全消散的帝国余孽……'", 4),
        ("'恐怕，那群自称蔷薇会的疯子们也要归来了。'", 2.5),
        ...
        ("'你妈的！'血裔的执政官痛骂：'你就是不信老娘！'", 2.5),
        ("教国，大圣教堂，神像下。", DELAY.VERY_SLOW),
        ("一名身穿白金色长袍的女子跪在地面，她闭着双眼，眼眸上蒙着白色的布，神色既圣洁，又慈悲。", 4),
...
def epilogue_sword_dojjo():
    ...
    DELAY.sleep(0.68 if condition else 1.67)
    p("'怎么了？'你回头问询。")
    p("'唔，路上小心。'她还是没说出口。")
    ...
    scenes2 = [
        ...
        ("柳生霜月将多余思绪抛下，这时再轻轻抚摸着手腕，白皙的手腕上什么都没有，浮现的云纹也悄然消退。", 5),
    ]
...
def epilogue_leave_school():
    scenes = [
        ("离开极东大学的校门时，你将证件递给门口的人员。", 2.5),
        ("这时眼角余光瞥见了一抹白色。", DELAY.VERY_SLOW),
        ("白发银眸。", DELAY.LONG),
        ("明国仙家从左方迎面而来，你顷刻间顿住，俨然大受震撼，但立刻掩盖住了神色，若无其事的右转。", 4),
        ("云无心停下脚步，看向前方人影，却因为你转身，只隐约看见一道眼熟的背影。", 3.5),
        ("'怎么了？'旁侧的薛寒泪问了句。", DELAY.VERY_SLOW),
        ("'……没事。'她立刻收回视线：'我们得快点找到这股气息的出处。'", 2.5),
